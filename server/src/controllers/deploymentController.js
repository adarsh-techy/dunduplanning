...
export const create = asyncHandler(async (req, res) => {
  const { title, category, description, who, date, renewalDate, cost, status, notes, order } = req.body;
  ...
  const item = await Deployment.create({
    ...
    status,
    notes,
    order: order ?? (await Deployment.countDocuments()),
...
export const update = asyncHandler(async (req, res) => {
  const item = await Deployment.findById(req.params.id);
  if (!item) {
    return res.status(404).json({ message: 'Item not found' });
  }

  const directFields = ['title', 'category', 'description', 'who', 'cost', 'status', 'notes', 'order'];
  directFields.forEach((field) => {
    if (req.body[field] !== undefined) item[field] = req.body[field];
  });
  const nullableDateFields = ['date', 'renewalDate'];
